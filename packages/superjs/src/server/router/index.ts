...
export class Router {
  ...
  private registerResourceRoutes(
    ...
    if (actions.index !== undefined) {
      this.get(basePath, this.createControllerHandler(controller, actions.index))
    }

    if (actions.create !== undefined) {
      this.get(`${basePath}/create`, this.createControllerHandler(controller, actions.create))
    }

    if (actions.store !== undefined) {
      this.post(basePath, this.createControllerHandler(controller, actions.store))
    }

    if (actions.show !== undefined) {
      this.get(`${basePath}/:${paramName}`, this.createControllerHandler(controller, actions.show))
    }

    if (actions.edit !== undefined) {
      this.get(`${basePath}/:${paramName}/edit`, this.createControllerHandler(controller, actions.edit))
    }

    if (actions.update !== undefined) {
      this.put(`${basePath}/:${paramName}`, this.createControllerHandler(controller, actions.update))
      this.patch(`${basePath}/:${paramName}`, this.createControllerHandler(controller, actions.update))
    }

    if (actions.destroy !== undefined) {
      this.delete(`${basePath}/:${paramName}`, this.createControllerHandler(controller, actions.destroy))
    }
