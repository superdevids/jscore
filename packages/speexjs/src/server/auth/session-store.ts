...
export class MemorySessionStore implements SessionStore {
  ...
  async read(id: string): Promise<Record<string, unknown> | null> {
    ...
    if (entry.expiresAt < Date.now()) { this.sessions.delete(id); return null }
    return entry.data
  }
...
export class DatabaseSessionStore implements SessionStore {
  constructor(private db: QueryRunner, private table: string = 'sessions') {}
  
  async read(id: string): Promise<Record<string, unknown> | null> {
    const dialect = this.db.getDialect()
    const result = await this.db.raw(
      `SELECT * FROM ${dialect.wrapIdentifier(this.table)} WHERE \`id\` = ? AND \`expires_at\` > ?`,
      [id, Date.now()]
    )
  ...
  async write(id: string, data: Record<string, unknown>, expiresAt: number): Promise<void> {
    ...
    await this.db.raw(
      `REPLACE INTO ${dialect.wrapIdentifier(this.table)} (\`id\`, \`data\`, \`expires_at\`) VALUES (?, ?, ?)`,
      [id, json, expiresAt]
    )
  }
  ...
  async destroy(id: string): Promise<void> {
    const dialect = this.db.getDialect()
    await this.db.raw(`DELETE FROM ${dialect.wrapIdentifier(this.table)} WHERE \`id\` = ?`, [id])
  }
  
  async cleanup(): Promise<void> {
    const dialect = this.db.getDialect()
    await this.db.raw(`DELETE FROM ${dialect.wrapIdentifier(this.table)} WHERE \`expires_at\` < ?`, [Date.now()])
  }
}
