...
import type {
	...
	JoinType,
	OrderDirection,
} from "../src/server/database/types.js";
...
function makeMockRunner(
	driver: DatabaseDriver = "mysql",
): { runner: QueryRunner; raw: ReturnType<typeof vi.fn> } {
...
describe("DatabaseConnection", () => {
	...
	describe("raw", () => {
		it("throws when not connected", async () => {
			const conn = new DatabaseConnection({ database: "test" });
			await expect(conn.raw("SELECT 1")).rejects.toThrow(
	...
	describe("transaction", () => {
		it("throws when not connected", async () => {
			const conn = new DatabaseConnection({ database: "test" });
			await expect(conn.transaction(async (trx) => "ok")).rejects.toThrow(
				"Database not connected",
			);
		});
		...
		it("calls driver.transaction and wraps callback", async () => {
			...
			await conn.connect();
			const driver = (conn as any).driver;
			driver.transaction = vi
				.fn()
				.mockImplementationOnce(async <T>(cb: (d: any) => Promise<T>) => {
					return cb(driver);
				});
	...
	describe("generateId", () => {
		...
		it("returns unique values", () => {
			const ids = new Set(
				Array.from({ length: 100 }, () => DatabaseConnection.generateId()),
			);
...
describe("Driver", () => {
	...
	describe("MysqlDriver", () => {
		...
		it("raw() throws when not connected", async () => {
			await expect(driver.raw("SELECT 1")).rejects.toThrow(
				"Database not connected",
			);
		});
		...
		it("raw() executes and returns rows", async () => {
			await driver.connect();
			mysqlMocks.pool.execute.mockResolvedValueOnce([
				{ insertId: 42 },
				[],
			]);
			const result = await driver.raw("INSERT INTO t (n) VALUES (?)", [1]);
			expect(mysqlMocks.pool.execute).toHaveBeenCalledWith(
...
describe("Dialect – advanced coverage", () => {
	describe("MysqlDialect", () => {
		...
		it("compileTableBlueprint", () => {
			...
			const result = d.compileTableBlueprint("users", cols, [
				"UNIQUE (name)",
			]);
	...
	describe("SqliteDialect", () => {
		...
		it("compileAddColumns", () => {
			...
			expect(d.compileAddColumns("users", [col])).toBe(
				'ADD COLUMN "age" INTEGER NULL',
			);
		});

		it("compileDropColumns", () => {
			expect(d.compileDropColumns("users", ["age", "bio"])).toBe(
				'DROP COLUMN "age", DROP COLUMN "bio"',
			);
		});

		it("compileRenameColumn", () => {
			expect(d.compileRenameColumn("users", "name", "username")).toBe(
		...
		it("compileRenameTable", () => {
			expect(d.compileRenameTable("old", "new")).toBe(
		...
		it("compileDropTableIfExists", () => {
			expect(d.compileDropTableIfExists("users")).toBe(
	...
	describe("PostgresqlDialect", () => {
		...
		it("compileDropTable", () => {
			expect(d.compileDropTable("users")).toBe(
				'DROP TABLE IF EXISTS "users"',
			);
		});
...
describe("Dialect – mapType branches for uncovered types", () => {
	...
	describe("PostgresqlDialect", () => {
		...
		it("compileColumn with string default value", () => {
			const sql = d.compileColumn(makeCol({
				name: "status", type: "string", nullable: false,
				defaultValue: "active", length: 50,
			}));
			expect(sql).toContain("DEFAULT 'active'");
		});
	});
...
describe("QueryBuilder – additional coverage", () => {
	...
	it("update with inRandomOrder and limit", async () => {
		...
		expect(raw).toHaveBeenCalledWith(
			"UPDATE `users` SET `status` = ? WHERE `status` = ? ORDER BY RANDOM() ASC LIMIT ?",
			["inactive", "active", 5],
		);
	});
	...
	it("delete with inRandomOrder and limit", async () => {
		...
		expect(raw).toHaveBeenCalledWith(
			"DELETE FROM `users` WHERE `status` = ? ORDER BY RANDOM() ASC LIMIT ?",
			["trash", 5],
		);
	});
});
