...
import {
	...
	PostgresqlDialect,
	createDialect,
} from "../src/server/database/dialect.js";
import {
	SchemaBuilder,
	TableBlueprint,
	ColumnDefinition,
	...
	Migrator,
} from "../src/server/database/migration.js";
...
describe("QueryBuilder", () => {
	...
	describe("WHERE clauses", () => {
		...
		it("orWhere preserves boolean OR correctly with mixed AND/OR", () => {
			...
			const { sql, bindings } = qb.toSQL();
			expect(sql).toBe(
				"SELECT * FROM `users` WHERE `status` = ? AND `age` > ? OR `name` = ?",
			);
		...
		it("whereGroup with complex nested boolean logic", () => {
			...
			const { sql, bindings } = qb.toSQL();
			expect(sql).toBe(
				"SELECT * FROM `posts` WHERE `published` = ? AND (`author_id` = ? OR `author_id` = ?)",
			);
		...
		it("mixed WHERE types produce correct SQL", () => {
			...
			const { sql } = qb.toSQL();
			expect(sql).toBe(
				"SELECT * FROM `users` WHERE `status` = ? AND `id` IN (?, ?) AND `deleted_at` IS NULL AND `name` LIKE ?",
			);
		});
		...
		it("critical bug test: OR where with AND should generate correct boolean", () => {
			...
			const { sql, bindings } = qb.toSQL();
			expect(sql).toBe(
				"SELECT * FROM `posts` WHERE `published` = ? AND `status` = ? OR `status` = ?",
			);
	...
	describe("ORDER BY", () => {
		...
		it("inRandomOrder", () => {
			...
			qb.inRandomOrder();
			const { sql } = qb.toSQL();
			expect(sql).toBe("SELECT * FROM `users` ORDER BY RANDOM() ASC");
		});
	...
	describe("GROUP BY / HAVING", () => {
		it("groupBy", () => {
			...
			const { sql } = qb.toSQL();
			expect(sql).toBe(
				"SELECT * FROM `users` GROUP BY `role`, `status`",
			);
		});
		...
		it("having", () => {
			...
			const { sql, bindings } = qb.toSQL();
			expect(sql).toBe(
				"SELECT * FROM `users` GROUP BY `role` HAVING `COUNT(*)` > ?",
			);
		...
		it("multiple havings are AND-ed", () => {
			...
			const { sql } = qb.toSQL();
			expect(sql).toBe(
				"SELECT * FROM `orders` GROUP BY `user_id` HAVING `SUM(amount)` > ? AND `COUNT(*)` >= ?",
			);
		});
	});
	...
	describe("UPDATE", () => {
		...
		it("update with orderBy and limit", async () => {
			...
			expect(raw).toHaveBeenCalledWith(
				"UPDATE `users` SET `status` = ? WHERE `status` = ? ORDER BY `id` ASC LIMIT ?",
				["active", "inactive", 1],
			);
		});
	});
	...
	describe("DELETE", () => {
		...
		it("delete with orderBy and limit", async () => {
			...
			expect(raw).toHaveBeenCalledWith(
				"DELETE FROM `users` WHERE `status` = ? ORDER BY `id` ASC LIMIT ?",
				["spam", 5],
			);
		});
	});
	...
	describe("PAGINATE", () => {
		it("paginate returns correct page 1", async () => {
			const { qb, raw } = makeQb("users");
			raw
				.mockResolvedValueOnce({ rows: [{ aggregate: 25 }] })
	...
	describe("CLONE", () => {
		it("clone returns independent copy", () => {
			...
			expect(cloneSql).toBe(
				"SELECT * FROM `users` WHERE `status` = ? AND `name` = ? LIMIT ?",
			);
		});
	...
	describe("MysqlDialect", () => {
		...
		it("compileColumn – string with default", () => {
			const sql = d.compileColumn({
				...
				defaultValue: "admin@test.com", unsigned: false, unique: false,
				primary: false, index: false, comment: null, after: null,
				first: false, autoIncrement: false, precision: null, scale: null,
				length: 255, values: null, isForeignId: false,
			});
		...
		it("compileColumn – nullable column", () => {
			const sql = d.compileColumn({
				...
				defaultValue: undefined, unsigned: false, unique: false,
				primary: false, index: false, comment: null, after: null,
				first: false, autoIncrement: false, precision: null, scale: null,
				length: null, values: null, isForeignId: false,
			});
		...
		it("compileCreateTable", () => {
			const columns = [
				{ name: "id", type: "increments", nullable: false, defaultValue: undefined, unsigned: false, unique: false, primary: false, index: false, comment: null, after: null, first: false, autoIncrement: false, precision: null, scale: null, length: null, values: null, isForeignId: false },
				{ name: "name", type: "string", nullable: false, defaultValue: undefined, unsigned: false, unique: false, primary: false, index: false, comment: null, after: null, first: false, autoIncrement: false, precision: null, scale: null, length: 255, values: null, isForeignId: false },
			];
		...
		it("compileAddColumns", () => {
			const col = {
				name: "age", type: "integer", nullable: true, defaultValue: undefined,
				unsigned: false, unique: false, primary: false, index: false,
				comment: null, after: null, first: false, autoIncrement: false,
				precision: null, scale: null, length: null, values: null, isForeignId: false,
			};
	...
	describe("SqliteDialect", () => {
		...
		it("compileColumn – integer nullable with default", () => {
			const sql = d.compileColumn({
				...
				defaultValue: 0, unsigned: false, unique: false,
				primary: false, index: false, comment: null, after: null,
				first: false, autoIncrement: false, precision: null, scale: null,
				length: null, values: null, isForeignId: false,
			});
	...
	describe("Migrator", () => {
		it("run() executes pending migrations", async () => {
			...
			migrator.setMigrations([
				{ name: "create_users_table", up, down },
			]);
			await migrator.run();
		...
		it("run() logs nothing to migrate when all ran", async () => {
			...
			const migrator = new Migrator(runner as any);
			migrator.setMigrations([
				{ name: "already_done", up: vi.fn(), down: vi.fn() },
			]);
		...
		it("status() returns ran migrations", async () => {
			...
			raw
				...
				.mockResolvedValueOnce({
					rows: [
						{ name: "m1", batch: 1, executedAt: "2024-01-01" },
					],
				});
			const migrator = new Migrator(runner as any);
...
describe("Seeder", () => {
	...
	it("run() throws if not overridden", async () => {
		...
		await expect(seeder.run()).rejects.toThrow(
			"Seeder.run() must be overridden by subclasses",
		);
	});
});
